...
| `BRANCH_FILE` | `feat-my-feature`                     | The branch name sanitized to a filename-safe format, any character outside `[a-zA-Z0-9._-]` is replaced with `-`.                                                                                                                                       |
| `BRANCH_PREFIX` | `feat`                               | The portion of the branch name before the first `/`, if any. Useful to group watermarks by branch naming convention, e.g. `feat/my-feature` becomes `feat`.                                                                                             |
| `TAG`    | `feat-1`                                   | The tag that triggered the workflow run.                                                                                                                                                                                                                 |
| `TAG_FILE` | `feat-1`                                 | The tag name sanitized to a filename-safe format, any character outside `[a-zA-Z0-9._-]` is replaced with `-`.                                                                                                                                          |
| `SHA`    | `2e6d...c932`                              | The hash of the commit that is being processed.                                                                                                                                                                                                          |
